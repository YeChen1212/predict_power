...
#白天/下午/晚上
filterData['time_of_day'] = filterData['hour'].apply(lambda x: 0 if 6 <= x < 12 else (1 if 12 <= x < 18 else 2))

train_data = filterData[~((filterData['day'] == 30) & (filterData['hour'] >= 9) & (filterData['hour'] <= 15))]
test_data = filterData[(filterData['day'] == 30) & (filterData['hour'] >= 9) & (filterData['hour'] <= 15)]

train_data.to_csv(r'D:\program\python\ML\ML_finalReport_train.csv', index=False)
test_data.to_csv(r'D:\program\python\ML\ML_finalReport_test.csv', index=False)
print(f'處理後 (訓練集): {train_data.shape}')
print(f'處理後 (測試集): {test_data.shape}')